...
1. [安装组件](#安装组件)
  1. [docker安装](#docker安装)
  1. [kubernetes管理软件安装](#kubernetes管理软件安装)
  1. [keepalived安装](#keepalived安装)
1. [创建配置文件](#创建配置文件)
  1. [生成相关配置文件](#生成相关配置文件)
  1. [配置文件清单](#配置文件清单)
1. [启动load-balancer](#启动loadbalancer)
  1. [启动keepalived](#启动keepalived)
  1. [启动nginx-lb](#启动nginxlb)
1. [初始化高可用master集群](#初始化高可用master集群)
  1. [安装第一个master节点](#安装第一个master节点)
  1. [把其他master节点加入controlplane控制平面](#把其他master节点加入controlplane控制平面)
1. [把nginx-lb作为kubernetes集群基础服务](#把nginxlb作为kubernetes集群基础服务)
1. [添加worker节点](#添加worker节点)
1. [安装组件](#安装组件)
  1. [kubernetes-dashboard](#kubernetes-dashboard)
    1. [安装kubernetes-dashboard](#安装kubernetes-dashboard)
    1. [登录kubernetes-dashboard](#登录kubernetes-dashboard)
    1. [使用kubernetes-dashboard管理集群](#使用kubernetes-dashboard管理集群)
  1. [heapster](#heapster)
    1. [安装heapster](#安装heapster)
    1. [验证heapster度量信息采集](#验证heapster度量信息采集)
  1. [metrics-server](#metrics-server)
    1. [安装metrics-server](#安装metrics-server)
  1. [prometheus](#prometheus)
    1. [安装prometheus相关组件](#安装prometheus相关组件)
    1. [使用prometheus监控性能](#使用prometheus监控性能)
    1. [使用alertmanager验证告警](#使用alertmanager验证告警)
  1. [grafana](#grafana)
    1. [安装grafana](#安装grafana)
    1. [使用grafana呈现prometheus性能指标](#使用grafana呈现prometheus性能指标)
  1. [istio](#istio)
    1. [安装istio](#安装istio)
    1. [使用istio进行AB测试](#使用istio进行AB测试)
    1. [进行服务跟踪](#进行服务跟踪)
    1. [进行流量监测](#进行流量监测)
  1. [traefik](#traefik)
    1. [安装traefik](#安装traefik)
    1. [使用traefik作为边界路由器](#使用traefik作为边界路由器)
1. [集群验证](#集群验证)
  1. [集群高可用验证测试](#集群高可用验证测试)
  1. [nodePort测试](#nodeport测试)
  1. [集群内服务测试](#集群内服务测试)
  1. [测试自动扩缩容](#测试自动扩缩容)
1. [证书到期更新](#证书到期更新)
